...
4. In the Extension Development Host, run **Browser: Browser Annotator for Codex** from the Command Palette.

Chrome, Chromium, Edge, or Brave must be installed. Common Windows, macOS, and Linux locations are detected, including per-user installs. For a custom path, set `browser-annotator-for-codex.chromeExecutable`; `~`, `${env:NAME}`, and Windows `%NAME%` variables are supported.

New browser tabs open with the address field focused and a dropdown of recent pages and other open browser tabs. When there are no recent pages yet, `http://localhost:3000` is offered as Home; change `browser-annotator-for-codex.homepage` to use a different development server.
...
The live viewport automatically uses VS Code's current display pixel ratio, so it stays sharp when the window is moved between monitors. Set `browser-annotator-for-codex.renderScale` to `1`–`4` only when you want to override automatic matching.
...
The extension checks the rendered Chromium page every `browser-annotator-for-codex.refreshInterval` milliseconds (1000 ms by default) and sends a new frame only when the pixels change. Browser interactions refresh immediately. Changes made by Codex appear automatically when the development server applies them through hot reload. If the development server does not support hot reload or is not running, rebuild the app and reload the browser page.
